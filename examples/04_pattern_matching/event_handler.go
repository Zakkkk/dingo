...
// Each enum variant becomes a struct implementing the interface.
// This enables exhaustive pattern matching with Go type switches.
package main
...
//
//line examples/04_pattern_matching//event_handler.dingo:17:1
...
//line examples/04_pattern_matching//event_handler.dingo:25:1
...
func ProcessEvent(event Event) string {
	//line examples/04_pattern_matching//event_handler.dingo:29:9
	val3 := event
	switch v4 := val3.(type) {
...
func GetEventPriority(event Event) int {
	//line examples/04_pattern_matching//event_handler.dingo:55:9
	val1 := event
	switch v2 := val1.(type) {
	case EventPaymentFailed:
		return 1
...
func FilterUserEvents(events []Event) []Event {
	...
	for _, event := range events {
		//line examples/04_pattern_matching//event_handler.dingo:67:18
